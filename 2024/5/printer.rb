...
def valid_order?(pages, rules)
  pages.each_with_index do |page, i|
    if rules[page]
      rules[page].each do |must_come_after|
        next unless pages.include?(must_come_after)
        after_idx = pages.index(must_come_after)
        return false if after_idx < i
      end
    end
  end
  true
...
def solve(input)
  ...
  return 0 unless blank_idx
  
  # Process updates from second section
  sum = 0
  lines[(blank_idx+1)..-1].each do |line|
    next if line.empty?
    pages = line.strip.split(',').map(&:to_i)
    if valid_order?(pages, rules)
      middle_idx = pages.length / 2
...
def solve_part2(input)
  ...
  lines[(blank_idx+1)..-1].each do |line|
    ...
    next if valid_order?(pages, rules)
    
    # Sort pages based on dependencies
    sorted = pages.sort do |a, b|
      if rules[a]&.include?(b)
        1 # a must come after b
      elsif rules[b]&.include?(a)
        -1 # b must come after a
      else
        b <=> a # Default to descending if no direct dependency
      end
    end
